...
class CallerAttributionTest {
  ...
  void returnsCallerPackage() {
    ...
    // The test is in io.jguard.agent, but that's in INFRASTRUCTURE_PACKAGES
  ...
  void skipsInfrastructurePackages() {
    ...
    // Since we're calling from io.jguard.agent (which is in the skip list),
    ...
    assertThat(pkg).isNotEqualTo("io.jguard.agent");
    assertThat(pkg).isNotEqualTo("io.jguard.core");
  }
}
