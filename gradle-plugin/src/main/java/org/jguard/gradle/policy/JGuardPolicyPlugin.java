...
/**
 * Gradle plugin that compiles {@code module-info.jguard} policy descriptors
 * and packages them into JAR files.
 *
 * <p>The plugin:
 * <ul>
 *   <li>Registers the {@code jguardPolicy} extension for configuration</li>
 *   <li>Creates a {@code compileJGuardPolicy} task</li>
 *   <li>If the Java plugin is applied, wires the output into the JAR</li>
 * </ul>
 *
...
public class JGuardPolicyPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        JGuardPolicyExtension extension = project.getExtensions()
            .create(EXTENSION_NAME, JGuardPolicyExtension.class);

        configureExtensionDefaults(project, extension);

        // Register the compile task
        TaskProvider<CompileJGuardPolicyTask> compileTask = project.getTasks()
            .register(TASK_NAME, CompileJGuardPolicyTask.class, task -> {
                task.setDescription("Compiles module-info.jguard into policy.bin and optional policy.json");
                ...
                task.getOutputBin().set(
                    outputDir.flatMap(dir -> extension.getBinName().map(dir::file))
                );
                task.getOutputJson().set(
                    extension.getIncludeJson().flatMap(include ->
                        include
                            ? outputDir.flatMap(dir -> extension.getJsonName().map(dir::file))
        ...
        project.getPlugins().withType(JavaPlugin.class, javaPlugin -> {
            project.getTasks().named(JavaPlugin.JAR_TASK_NAME, Jar.class, jar -> {
                ...
                jar.from(compileTask.flatMap(task -> {
